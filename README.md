...
Unlike the Original Nintendo Game Boy, Vorpon runs on a Dual-Band Virtual Processor which lets the BIOS do some tests on both the BIOS Configuration, and the game itself. Vorpon is only supported for Windows as of now, but there is limited support for both Linux/macOS. Full support for both Linux/macOS will be released after sometime of the first release. If your Vorpon bootloops on the "Booting Game..." screen, please make a "games" folder inside the Vorpon Root Folder and put all the games for Vorpon in the "games" folder. If you have no games, the screen will change to a "No Cartridge Detected" screen.
